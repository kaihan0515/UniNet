...
def overlay_panel(img_path, amap, gt_path, vmin, vmax):
    orig = cv2.imread(img_path)
    orig = cv2.resize(orig, (DISP, DISP))
    am = gaussian_filter(amap, sigma=4)
    am = (am - vmin) / (vmax - vmin + 1e-8)
    am = np.clip(am, 0, 1)
    am = cv2.resize(am, (DISP, DISP))
...
def main():
    ...
    # ---- pass 1: compute all anomaly maps, collect a GLOBAL color scale ----
    items = []          # (cat, img_path, gt_path, amap, score)
    pooled = []
    for cat in cats:
        ...
        for ip in imgs[:n]:
            ...
            items.append((cat, ip, gt, amap, score))
            pooled.append(gaussian_filter(amap, sigma=4).ravel())
    ...
    per_cat_cnt = {}
    for cat, ip, gt, amap, score in items:
        out_dir = os.path.join(OUT, cat)
        os.makedirs(out_dir, exist_ok=True)
        stem = os.path.splitext(os.path.basename(ip))[0]
        panel = overlay_panel(ip, amap, gt, vmin, vmax)
