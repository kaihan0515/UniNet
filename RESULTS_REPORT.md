...
針對「小瑕疵(如小黑傷)被反光紋路蓋過」的問題,系統性嘗試三種設定並比較:

| 版本 | 方法 | image AUROC | pixel AUROC | pixel AUPRO |
|------|------|:-----------:|:-----------:|:-----------:|
| **SteelBall** | 基準:未對齊 + 無監督 | 100 | 84.4 | 58.6 |
| **SteelBallA** | (A) 球體對齊 + 無監督 | 100 | 83.3 | 54.1 |
| **SteelBallSyn** | (A)+(B) 對齊 + 合成小瑕疵監督 | 100 | **87.9** | 47.6 |
| **SteelBallPatch** | (C) 高解析 4×4 patch + 拼回 | 99.9 | 78.6 | **75.7** |

> 各版「測試集/評估方式」略有差異(Patch 為對齊後切塊拼回),數值為各自最佳值,趨勢為主。

![四版對照](report/figs/version_comparison.png)

**觀察與結論**:
- **image AUROC**:四版皆 ≈ 100(OK/NG 判定都很好;Patch 略降到 99.9)。
- **pixel AUROC**:**(B) 合成監督最高(87.9)**。
- **pixel AUPRO(區域定位,對小瑕疵最關鍵)**:**(C) Patch 大幅領先(75.7,其餘 ~48–59)** —— 高解析 patch 讓被抓到的瑕疵區域覆蓋更準。
- 但**質化檢視**:四版的 100 小黑傷熱圖**仍會被中央反光紋路帶走**,patch 也未根除。
- 根本原因:**鏡面反光 facet 與小瑕疵在外觀/尺度/頻率上重疊**;軟體方法(淺層特徵、頻率前處理、對齊、合成監督、patch)各有取捨、皆非根治。
- **真正解法在成像端**:**漫射圓頂光或偏振片**消除鏡面反光,或大量良品涵蓋各反光轉向。

> **小結**:若重視**瑕疵定位品質(AUPRO)**選 **Patch**;重視**像素分辨(pixel AUROC)**選 **合成監督**;純 OK/NG 檢測四版都達標。
