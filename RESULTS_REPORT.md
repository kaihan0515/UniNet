...
| 版本 | 方法 | 測試集 | image AUROC | pixel AUROC | pixel AUPRO |
|------|------|--------|:-----------:|:-----------:|:-----------:|
| **SteelBall** | 基準:未對齊 + 無監督 | 未對齊 | 100 | 84.4 | **58.6** |
| **SteelBallA** | (A) 球體對齊 + 無監督 | 對齊 | 100 | 83.3 | 54.1 |
| **SteelBallSyn** | (A)+(B) 對齊 + 合成小瑕疵監督 | 對齊 | 100 | **87.9** | 47.6 |

> SteelBallA 與 SteelBallSyn 同在「對齊測試集」,可直接比較。

![三版對照](report/figs/version_comparison.png)

**觀察與結論**:
- 三者 **image AUROC 皆 = 100**(OK/NG 判定都完美)。
- **(B) 合成監督**把 **pixel AUROC 拉高(83→88)**,但 **AUPRO 反而下降**,且質化檢視熱圖**仍會被反光紋路帶走**。
- 根本原因:**鏡面反光 facet(暗線)與小瑕疵在外觀/尺度/頻率上重疊**,純軟體方法(淺層特徵、頻率前處理、對齊、合成監督)都只有邊際改善。
- **真正解法在成像端**:改用**漫射圓頂光或偏振片**消除鏡面反光,或大量良品涵蓋各反光轉向。此為後續建議方向。
